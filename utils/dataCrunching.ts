...
export function getMealStrings(days: RawDay[]): string[] {
	return _.flattenDeep(
		days.map(day =>
			day.displayNames
				.filter(dn => dn.displayNameCategory.displayNameCategoryName === "English")
...
export function getIngredients(mealStrings: string[]) {
	...
	return _.flattenDeep(
		mealStrings.map(food => food.split(separatorRegex).filter(val => !separatorRegex.test(val))),
	)
		.map((ingredient: string) => ingredient.trim().toLowerCase())
		.sort((a, b) => ("" + a[0]).localeCompare(b[0]))
}

export function getGeneralMetaData(mealsAccumulated: Record<string, number>): GeneralMetaData {
	const average =
